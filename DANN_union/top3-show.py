...
def pymol_show(ac,pos_li,pos_li2):
    ...
    # 添加分子结构
    pdb_data = open(f"/data/liyifan/片段计算/esm/单个序列input/{ac}.pdb").read()
    ...
    with open(f"/data/liyifan/DANN_union/单个序列推荐结果/{ac}_important_show.html", "w") as html_file:
...
def text_to_image(text, font_path='/data/liyifan/片段计算/esm/ARIAL.TTF', font_size=20, output_path='output.png',set_figure =(1005,1000),seg=[],seg2=[]):
    # 创建一个白色背景的图片
...
seq_show = load_mydict('/data/liyifan/DANN_union/可视化中转/seq_show')
text = seq_show[0]
...
first_re = load_mydict('/data/liyifan/DANN_union/可视化中转/first_recom')
last_re = load_mydict('/data/liyifan/DANN_union/可视化中转/last_recom')
ac = first_re[0]
...
# print(first_re)
# print(last_re)
path = f'/data/liyifan/DANN_union/单个序列推荐结果/{ac}-animal.png'
